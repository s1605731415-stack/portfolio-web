        ...
        <div className="rounded-sm border border-[var(--line)] bg-[var(--surface)] p-6">
          ...
          <div className="mt-6 grid gap-3">
            {["Color / semantic states", "Type scale / hierarchy", "Spacing / density", "Components / variants"].map((item) => (
              <div className="flex items-center justify-between rounded-sm border border-[var(--line)] bg-[var(--bg-strong)] px-4 py-3" key={item}>
        ...
        <div className="rounded-sm border border-[var(--line)] bg-[var(--surface)] p-6">
          ...
          <div className="mt-6 space-y-4">
            {["States are named before visual polish.", "Responsive rules are defined with component behavior.", "AI workflow outputs are reviewed before they become design decisions."].map((item) => (
              <p className="rounded-sm bg-[var(--soft)] p-4 text-[14px] font-normal leading-[1.6] text-[var(--muted)]" key={item}>
                {item}
              </p>
            ))}
          </div>
